...
function ensureStyle() {
  ...
  if (!document.getElementById(EMOJI_FONT_LINK_ID)) {
    ...
    link3.id = EMOJI_FONT_LINK_ID;
    link3.rel = "stylesheet";
    link3.href = "https://fonts.googleapis.com/css2?family=Noto+Color+Emoji&display=swap";
...
function splitEmoji(s2) {
  const m3 = s2.match(EMOJI_RE);
  return m3 ? { emoji: m3[1], rest: s2.slice(m3[0].length) } : { emoji: "", rest: s2 };
}
var _measureCtx = (() => {
...
function wrapAndTruncate(text, maxChars, maxLines, ellipsis, overflowMode, tolerance) {
  ...
  const tokens = t.split(/(\s+)/).filter((s2) => s2.length > 0);
  const lines = [];
  let cur = "";
  const flush = () => {
    if (cur.length) {
...
function computeWordDisplay(d, k2, opts) {
  const baseFs = d.fs;
  ...
  const targetVisualMul = Math.max(0.1, 1 + (k2 - 1) * visualGrowth);
  const displayFs = baseFs * targetVisualMul / k2;
...
function rectCollide(padding, iterations2) {
  let nodes;
  function force() {
    for (let it = 0; it < iterations2; it++) {
      const tree = d3.quadtree(nodes, (d) => d.x, (d) => d.y);
      for (const a4 of nodes) {
        if (a4.fx != null) continue;
        const ar = Math.max(a4.w, a4.h) / 2 + padding + 60;
        tree.visit((node, x06, y06, x12, y12) => {
          if (!node.length) {
            do {
              const b = node.data;
              if (b !== a4) {
                const dx = b.x - a4.x;
                const dy = b.y - a4.y;
                const minX = (a4.w + b.w) / 2 + padding;
                const minY = (a4.h + b.h) / 2 + padding;
          ...
          return x06 > a4.x + ar || x12 < a4.x - ar || y06 > a4.y + ar || y12 < a4.y - ar;
        });
      }
    }
  }
...
function buildHullPath(items, opts) {
  ...
  if (!hull) {
    ...
    const minY = d3.min(ys), maxY2 = d3.max(ys);
    hull = [[minX, minY], [maxX, minY], [maxX, maxY2], [minX, maxY2]];
  }
  hull = hull.map(([x4, y4]) => {
    const dx = x4 - cx, dy = y4 - cy;
    const dist2 = Math.hypot(dx, dy);
    if (dist2 < minR) {
      const nx = dx === 0 && dy === 0 ? 1 : dx / (dist2 || 1);
      const ny = dx === 0 && dy === 0 ? 0 : dy / (dist2 || 1);
  ...
  hull = hull.map(([x4, y4]) => {
    ...
    return [x4 + dx / dist2 * inflate, y4 + dy / dist2 * inflate];
  });
  hull = hull.map(([x4, y4]) => {
    const dx = x4 - cx, dy = y4 - cy;
    const dist2 = Math.hypot(dx, dy) || 1;
    return [x4 + dx / dist2 * dist2 * 0.18, y4 + dy / dist2 * dist2 * 0.18];
  });
...
function mergeOptions(defaults, overrides) {
  ...
  for (const k2 in overrides) {
    const v2 = overrides[k2];
    if (v2 && typeof v2 === "object" && !Array.isArray(v2) && defaults[k2] && typeof defaults[k2] === "object") {
      out[k2] = Object.assign({}, defaults[k2], v2);
    } else if (v2 !== void 0) {
      out[k2] = v2;
    }
  }
...
var WordmapForce = class {
  ...
  _buildDOM() {
    ...
    const svg2 = d3.select(this.container).append("svg").attr("viewBox", `0 0 ${width} ${height}`).attr("preserveAspectRatio", "xMidYMid meet");
    this.svg = svg2;
    this.root = svg2.append("g").attr("class", "wf-root");
    ...
    if (zoomable) {
      ...
      svg2.call(this.zoomBehavior);
    }
  }
  _onZoom(k2) {
    if (Math.abs(k2 - this._lastZoomK) < this.opts.wordZoomRewrapEpsilon) return;
    this._lastZoomK = k2;
    if (this._wordSel) this._rerenderWordLines(k2);
  }
  _rerenderWordLines(k2) {
    const opts = this.opts;
    const FONT_KO = opts.fontFamilyKo;
    const wordTextByCi = this._wordTextByCi;
    this._wordSel.each(function(d) {
      const display = computeWordDisplay(d, k2, opts);
  ...
  render(data, extras = {}) {
    ...
    const c1FillByCi = c1Set.map((label, ci) => {
      ...
      if (sentScale) {
        let s2 = scoreC1 ? scoreC1[label] : null;
        if (s2 == null && cj != null && scoreC2) s2 = scoreC2[c2Set[cj]];
        if (s2 == null && sentFallback != null) s2 = sentFallback;
        if (s2 != null) return sentScale(s2);
      }
    ...
    const c2FillByCj = c2Set.map((label, cj) => {
      ...
      if (cu2 != null) return cu2;
      if (sentScale) {
        let s2 = scoreC2 ? scoreC2[label] : null;
        if (s2 == null && sentFallback != null) s2 = sentFallback;
        if (s2 != null) return sentScale(s2);
      }
      return baseColors[(cj % baseColors.length + baseColors.length) % baseColors.length];
    });
    const toWordColor = (c6) => {
      const hcl3 = d3.hcl(c6);
      hcl3.c = Math.max(hcl3.c, 30) * 1.4;
      hcl3.l = opts.wordTextLightness != null ? opts.wordTextLightness : 45;
      return hcl3.formatHex();
    };
    const toC1LabelColor = (c6) => {
      const hcl3 = d3.hcl(c6);
      const ch = opts.c1LabelChroma != null ? opts.c1LabelChroma : 32;
      const li = opts.c1LabelLightness != null ? opts.c1LabelLightness : 30;
      return d3.hcl(hcl3.h, ch, li).formatHex();
    };
    ...
    const toPillDark = (c6) => {
      const h = d3.hcl(c6);
      let hue2 = h.h, l = opts.wordTextLightness != null ? opts.wordTextLightness : 45;
      if (hue2 > 70 && hue2 < 130) {
        hue2 = 72;
        l = Math.max(l, 52);
      }
      return d3.hcl(hue2, Math.max(h.c, 30) * 1.4, l);
    };
    const c2PillByCj = c2FillByCj.map((c6) => d3.rgb(d3.interpolateHcl(c6, toPillDark(c6))(c2PillMix)).formatHex());
    ...
    c1Area.forEach((a4, ci) => c1Radius.set(ci, Math.sqrt(a4 / (Math.PI * 0.55)) + 8));
    ...
    childByC2.forEach((list, cj) => {
      const sumR = list.reduce((a4, d) => a4 + (c1Radius.get(d.ci) || 30), 0);
      const N = list.length;
    ...
    if (c2CollideStrength > 0) {
      ...
      if (stretch > 0 && c2Nodes2.length > 1) {
        ...
        for (const n of c2Nodes2) {
          ...
          if (n.y > maxY2) maxY2 = n.y;
        }
        const curW = Math.max(1, maxX - minX), curH = Math.max(1, maxY2 - minY);
        ...
        const cx = (minX + maxX) / 2, cy = (minY + maxY2) / 2;
        for (const n of c2Nodes2) {
          n.x = (n.x - cx) * sx + cx;
          n.y = (n.y - cy) * sy + cy;
        }
    ...
    const wordNodes = rows.map((d) => {
      ...
      const w = Math.max(...lineWidths) + 4;
      const h = ml * fs * 1.2;
      const c6 = c1Center.get(ci) || { x: opts.width / 2, y: opts.height / 2, r: 50 };
      return {
        type: "word",
        text: d.text,
        size: d.size || 1,
    ...
    const c1Count = d3.rollup(wordNodes, (v2) => v2.length, (d) => d.c1);
    const wordsByC1 = d3.group(wordNodes, (d) => d.c1);
    const GOLDEN = Math.PI * (3 - Math.sqrt(5));
    wordsByC1.forEach((items, ci) => {
      const c6 = c1Center.get(ci);
      if (!c6) return;
      items.sort((a4, b) => b.size - a4.size);
      const N = items.length;
      items.forEach((n, i) => {
        const ang = i * GOLDEN;
        const rad = Math.sqrt((i + 0.5) / N) * c6.r * 0.92;
        n.cx = c6.x + Math.cos(ang) * rad;
        n.cy = c6.y + Math.sin(ang) * rad;
        n.x = n.cx;
        n.y = n.cy;
      });
    });
    ...
    wordsByC1.forEach((words, ci) => {
      const v2 = words.reduce((s2, w) => s2 + (w.size || 1), 0);
      c1Value.set(ci, v2);
      totalValue += v2;
    });
    ...
    c1Center.forEach((c6, ci) => {
      ...
      if (opts.c1FontSize != null) {
        ...
        const min4 = opts.c1FontMin || 14;
        const cap = opts.c1FontMax != null ? opts.c1FontMax : Infinity;
        fs = Math.min(cap, Math.max(min4, auto));
      }
      c1FsByCi.set(ci, fs);
    ...
    const c2WordCount = d3.rollup(wordNodes, (v2) => v2.length, (d) => d.c2);
    ...
    const areaScores = [...c2Meta.values()].map((m3) => m3.areaScore);
    ...
    const c1ByC2 = d3.group([...c1Center.entries()].map(([ci, c6]) => ({ c1: ci, c: c6 })), (d) => c1ToC2.get(d.c1));
    ...
    c1ByC2.forEach((children2, ci) => {
      ...
      if (!meta) return;
      const c2value = children2.reduce((a4, ch) => a4 + (c1Value.get(ch.c1) || 0), 0);
      ...
      const childC1Fs = children2.map((ch) => c1FsByCi.get(ch.c1) || 0);
    ...
    c2Sel.each(function(d) {
      ...
      d.lines.forEach((line, i) => {
        const y4 = i * d.lineH - totalH / 2 + 1;
        const t = g.append("text").attr("y", y4).attr("font-family", FONT_EMOJI).attr("text-anchor", "middle").attr("dominant-baseline", "central");
        if (i === 0 && d.emoji) {
  ...
  fitToContent(padding) {
    ...
    for (const n of all) {
      ...
      if (n.y + halfH > maxY2) maxY2 = n.y + halfH;
    }
    ...
    maxX += totalPad;
    maxY2 += totalPad;
    const bw = maxX - minX, bh = maxY2 - minY;
    if (bw <= 0 || bh <= 0) return;
    const fit2 = Math.min(W / bw, H / bh);
    const [zMin, zMax] = this.opts.zoomExtent;
    if (fit2 < zMin) {
      this.zoomBehavior.scaleExtent([Math.min(zMin, fit2), zMax]);
    }
    const k2 = Math.min(fit2, zMax);
    const cx = (minX + maxX) / 2, cy = (minY + maxY2) / 2;
    const tx = W / 2 - cx * k2;
    const ty = H / 2 - cy * k2;
    const t = d3.zoomIdentity.translate(tx, ty).scale(k2);
    this.svg.call(this.zoomBehavior.transform, t);
    this._lastZoomK = k2;
  }
  _computeAnchors(c1Set, c2Set, c1ToC2, positions) {
    const opts = this.opts;
    const { width: W, height: H, margin: M2, clusterLocalScale } = opts;
    ...
    if (c2Pos.size === c2Set.length) {
      ...
      const sx = d3.scaleLinear().domain(d3.extent(xs)).range([M2.left + PAD.x, W - M2.right - PAD.x]);
      const sy = d3.scaleLinear().domain(d3.extent(ys)).range([M2.top + PAD.y, H - M2.bottom - PAD.y]);
      c2Pos.forEach((p, i) => c2Abs.set(i, { x: sx(p.x), y: sy(p.y) }));
    } else {
      const N = c2Set.length;
      const cols = Math.ceil(Math.sqrt(N * (W / H)));
      const rows = Math.ceil(N / cols);
      const cw = (W - M2.left - M2.right) / cols;
      const ch = (H - M2.top - M2.bottom) / rows;
      c2Set.forEach((_, i) => {
        const r = Math.floor(i / cols), c6 = i % cols;
        c2Abs.set(i, { x: M2.left + cw * (c6 + 0.5), y: M2.top + ch * (r + 0.5) });
      });
    }
  ...
  _applyStoredSettings() {
    ...
    const s2 = this._loadSettings();
    for (const spec of WF_SETTINGS_SLIDERS) {
      const raw = s2[spec.key];
      const v2 = Number(raw);
      if (raw == null || raw === "" || !Number.isFinite(v2)) continue;
  ...
  _buildSettingsPanel() {
    ...
    const specMap = new Map(WF_SETTINGS_SLIDERS.map((x4) => [x4.key, x4]));
...
// force-bubble.standalone.entry.js
if (typeof globalThis !== "undefined" && !globalThis.d3) globalThis.d3 = src_exports;
export {
  WordmapForce,
  createForceBubble,
